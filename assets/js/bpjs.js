...
const get_total = (kode) => {
  $.ajax({
    ...
    success: function (data) {
      ...
      total_iuran = ks + ktk;
      $(".judul_total").append(
        '<b class="text-success"> Rp.' +
          money(total_iuran) +
          "</b>" +
          "<small><b> ( Jumlah karyawan : " +
          data.total_orang +
          " orang )</b></small>"
      );
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (i = 0; i < data.length; i++) {
        ...
        html +=
          `  <tr>
        <input type="hidden" class="idDetil"  value="` +
          data[i].id +
          `">
                                  <td class="text-center">
                                      <div class="btn-group">
                                          <a class="item-edit mr-3" href="javascript:;" data="` +
          data[i].id +
          `"><i class="fa fa-pencil  text-success"></i></a>
                                          <a class=" item-delete" href="javascript:;" data="` +
          data[i].id +
          `" data-nama="` +
          data[i].nama +
          ...
          money(total) +
          `</td>
                                  <td class="text-center"><label style="font-size:7pt;">` +
          data[i].name +
          `</label></td>
                              </tr > `;
      }
      ...
      for (i = 0; i < data.length; i++) {
        html +=
          `  <tr>
                                  <td class="text-center">
                                      <div class="btn-group">
                                          <a class="item-copy mr-3" href="javascript:;" data="` +
          data[i].kode_bpjs +
          `" data-ket="` +
          data[i].ket_bpjs +
          `"><i class="fa fa-copy  text-success"></i></a>
                                      </div >
                                  </td >
                                  <td class="text-center">` +
          data[i].ket_bpjs +
    ...
    success: function (data) {
      ...
      for (i = 0; i < data.length; i++) {
        html +=
          `  <tr>
                      <td>
                          <input type="hidden"  name="id_kry_copy[]"  value="` +
          data[i].id_kry_bpjs +
          `">
                          <input type="hidden" " name="ks[]"  value="` +
          data[i].bpjs_kesehatan +
          `">
                          <input type="hidden" " name="ktk[]"  value="` +
          data[i].bpjs_ktk +
          `">
                          <input type="hidden" " name="tmb[]"  value="` +
          data[i].tambahan +
          `">
...
if (flashdata == "sama") {
  $(".alert-danger")
    .html("Ops....gagal Copy, Priode ini sudah ada !")
    .fadeIn()
    .delay(3000)
    .fadeOut("slow");
} else if (flashdata == "berhasil") {
  $(".alert-success")
    .html("Berhasil Copy data !")
    .fadeIn()
    .delay(3000)
    .fadeOut("slow");
}
